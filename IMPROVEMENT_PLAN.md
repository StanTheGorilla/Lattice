...
### P0-2 Nightly SQLite backup job
...
### P1-1 Timestamp comparison audit (cross-cutting)
...
### P1-2 `sleep_debt.py` must reuse F4's healthy envelope
...
### P1-3 Unify "what counts as caffeinated" between F4 and F5
...
### P1-4 F5 misses after-midnight caffeine
...
### P1-5 F5: add a daily caffeine cap
...
### P1-6 `allostatic_load.py` adjustments
...
### P1-7 Harden the algorithm sandbox (`llm/sandbox.py`)
...
### V-1 `health_targets` keyed store
...
### V-2 Deterministic functions read the store
...
### V-3 Wide outer guardrails (few, medically defensible — not rules)
